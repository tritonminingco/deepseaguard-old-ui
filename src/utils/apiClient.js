...
import { getMockData, simulateWebSocket } from "./mockData";
...
export function useHistoricalData(endpoint, timeFrame, options = {}) {
  ...
  const cacheKey = `${endpoint}_${timeFrame}_${JSON.stringify(
  ...
  const fetchHistoricalData = useCallback(
    async (force = false) => {
      ...
      if (
        ...
        dataCache.current[cacheKey].timestamp
      ) {
      ...
      try {
        ...
        if (true) {
          console.log(
            `Using mock data for historical endpoint: ${endpoint}, timeFrame: ${timeFrame}`
          );
